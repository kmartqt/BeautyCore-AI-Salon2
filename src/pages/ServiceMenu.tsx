...
const menuData: Record<string, ServiceMenuCategory> = {
  spa: {
    heroTitle: "Japanese Head Spa",
    heroSubtitle: "Scalp Detox & Relaxation at Andrea's Wellness Clinic",
    heroDesc:
      "Premium Josiane Laure products from France and Milbon technology from Japan to deliver a truly results-driven and luxurious head spa experience.",
  ...
  hair: {
    ...
    heroDesc:
      "Our stylists are highly trained and continually educate themselves in precision and razor cuts, hair coloring, corrective coloring, straightening, keratin, permanent waves and customized hair conditioning.",
    heroImage: "https://images.unsplash.com/photo-1741816219998-be418d9cd1fc",
    sections: [
      {
        title: "Haircut & Style",
        description:
          "Our stylists are also accomplished colorists, which makes it easier to communicate a total vision of your coloring and styling desires.",
        items: [
          {
            name: "Women's Cut & Style",
            desc: "Precision haircut and style where your look will fit your personality and lifestyle.",
            price: "₱2,800",
          },
          {
            name: "Women's Cut",
            desc: "Our hair artists skillfully integrate the latest trends and timeless classics.",
            price: "₱2,200",
          },
          { name: "Blowout & Style", desc: "More than just a Style. This treatment will include a bang trim as well.", price: "₱1,500" },
          {
            name: "Curly to Straight",
            desc: "Need a quick-fix. Just relax. This choice treatment includes hair conditioner and ceramic flat smoothing.",
            price: "₱3,500",
          },
          {
            name: "Up-Do Styling",
            desc: "Each experience begins with an 'in-chair' consultation.",
            price: "₱2,800",
          },
        ],
      },
  ...
  face: {
    ...
    heroDesc:
      "Advanced aesthetic treatments utilizing state-of-the-art laser technology and medical-grade skincare to address aging, pigmentation, and overall skin health.",
  ...
  nails: {
    ...
    heroDesc:
      "Experience meticulous nail care in a deeply relaxing environment. We use only premium, non-toxic polishes and rigorous sanitization standards.",
  ...
  massage: {
    ...
    heroDesc:
      "Melt away tension and restore muscular balance with our bespoke massage therapies, performed by expert therapists using organic botanical oils.",
    ...
    sections: [
      {
        ...
        items: [
          {
            name: "Swedish Relaxation (60/90 Min)",
            desc: "Gentle, flowing strokes to improve circulation and reduce stress.",
            price: "₱2,500 / ₱3,500",
          },
          {
            name: "Deep Tissue Rescue (60/90 Min)",
            desc: "Targeted pressure to relieve chronic muscle tension.",
            price: "₱2,800 / ₱3,800",
          },
          { name: "Hot Stone Therapy (90 Min)", desc: "Smooth, heated stones to melt away deep-seated knots.", price: "₱4,500" },
          {
            name: "Aromatherapy Journey (90 Min)",
            desc: "Customized essential oil blend tailored to your mood.",
            price: "₱3,800",
          },
        ],
      },
...
export function ServiceMenu() {
  ...
  const ServiceCategoryTemplate = ({ id, data }: { id: string; data: ServiceMenuCategory }) => (
    <section id={id} className="scroll-mt-32">
      <div className="relative w-full h-[60vh] min-h-[500px] mb-16 overflow-hidden flex items-end">
        <ImageWithFallback src={data.heroImage} alt={data.heroTitle} className="absolute inset-0 w-full h-full object-cover object-top" />
              ...
              {section.items.map((item, itemIdx) => (
                <div key={itemIdx} className="flex flex-col space-y-2 group">
                  <div className="flex justify-between items-baseline gap-4">
                    ...
                    <span className="font-sans text-sm font-medium text-zinc-900 whitespace-nowrap">{item.price}</span>
                  </div>
                  <p className="font-sans text-xs text-zinc-500 leading-relaxed pr-8">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
    ...
    <div className="w-full bg-[#F9F6F0] min-h-screen text-zinc-900 pt-24">
      <div className="w-full h-[40vh] min-h-[300px] relative bg-zinc-900 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4">
          <h1 className="font-serif text-4xl md:text-6xl tracking-[0.15em] text-[#F9F6F0] uppercase font-light mb-4">
            Our Services
          </h1>
      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 py-16 md:py-24 flex flex-col lg:flex-row gap-16 lg:gap-24">
          ...
          <div className="sticky top-32 flex flex-col space-y-2">
            ...
            {categories.map((cat) => (
              <a
                key={cat.id}
                href={`#${cat.id}`}
                className="font-sans text-sm tracking-[0.1em] text-zinc-600 py-2 px-3 border-l-2 border-transparent hover:border-[#D4AF37] hover:text-zinc-900 transition-colors"
              >
                {cat.label}
              </a>
            ))}
        ...
        <div className="flex-1 space-y-24">
          {categories.map((category) => (
            <ServiceCategoryTemplate key={category.id} id={category.id} data={menuData[category.id as keyof typeof menuData]} />
          ))}
        </div>
      </div>
    </div>
