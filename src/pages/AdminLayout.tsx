  ...
  return (
    <Link
      ...
      className={`flex items-center gap-3 px-4 py-2 text-sm transition-all ${
        active ? "bg-[#FAFAF9] text-[#1C1C1C] border-l-2 border-[#C4A47C]" : "text-[#8C8A84] hover:text-[#1C1C1C]"
      } ${adminOnly ? "opacity-80" : ""}`}
    >
      {icon}
      <span className="flex-1">{label}</span>
      {adminOnly && <span className="text-[10px] uppercase tracking-wider text-[#C4A47C]">Admin</span>}
    </Link>
  );
}
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="h-16 border-b border-[#E8E6E1] bg-white flex items-center justify-between px-8 shrink-0">
          <div className="flex items-center gap-4">
            <Link
              to="/"
              className="flex items-center gap-2 text-sm text-[#8C8A84] hover:text-[#1C1C1C] transition-colors"
            >
              <ChevronLeft size={16} />
            ...
            <div className="flex items-center bg-[#F5F5F0] rounded-full p-1 border border-[#E8E6E1]">
              <button
                ...
                className={`px-4 py-1.5 text-xs font-medium rounded-full transition-all ${
                  role === "staff" ? "bg-white shadow-sm text-[#1C1C1C]" : "text-[#8C8A84] hover:text-[#1C1C1C]"
                }`}
              >
                Staff
              </button>
              <button
                ...
                className={`px-4 py-1.5 text-xs font-medium rounded-full transition-all ${
                  role === "admin" ? "bg-white shadow-sm text-[#1C1C1C]" : "text-[#8C8A84] hover:text-[#1C1C1C]"
                }`}
              >
                Admin
              </button>
