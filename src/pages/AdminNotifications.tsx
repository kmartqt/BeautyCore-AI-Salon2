    ...
    <div className="max-w-4xl mx-auto space-y-8 h-full flex flex-col">
      <div className="flex items-end justify-between shrink-0">
        <div>
          ...
          <p className="text-sm text-[#8C8A84]">Stay updated with bookings, alerts, and system messages.</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white border border-[#E8E6E1] p-4 shrink-0">
        <div className="flex bg-[#F5F5F0] border border-[#E8E6E1] p-1 w-full sm:w-auto">
          <button className="px-4 py-1.5 text-xs font-medium bg-white shadow-sm text-[#1C1C1C] flex-1 sm:flex-none">
            All
          </button>
          <button className="px-4 py-1.5 text-xs font-medium text-[#8C8A84] hover:text-[#1C1C1C] flex-1 sm:flex-none">
            Unread (2)
          </button>
          <button className="px-4 py-1.5 text-xs font-medium text-[#8C8A84] hover:text-[#1C1C1C] flex-1 sm:flex-none">
            Alerts
          ...
          <div className="divide-y divide-[#E8E6E1]">
            {notificationsData.map((note) => {
              ...
              return (
                <div
                  key={note.id}
                  className={`p-6 flex gap-4 hover:bg-[#FAFAF9] transition-colors ${
                    note.unread ? "bg-[#FAFAF9]" : "bg-white"
                  }`}
                >
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h4 className={`text-sm font-medium text-[#1C1C1C]`}>{note.title}</h4>
                        <p className={`text-sm mt-1 ${note.unread ? "text-[#1C1C1C]" : "text-[#8C8A84]"}`}>{note.message}</p>
                      </div>
                      <span className="text-xs text-[#8C8A84] whitespace-nowrap">{note.time}</span>
                    </div>
                  </div>
                  <div className="flex items-start">{note.unread && <div className="w-2.5 h-2.5 rounded-full bg-[#C4A47C] mt-2" />}</div>
                </div>
              );
            })}
          </div>
